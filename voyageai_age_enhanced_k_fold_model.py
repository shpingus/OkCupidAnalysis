...
"""
VoyageAI Age Prediction Enhanced K-Fold Ensemble Model
...
with advanced feature engineering techniques adapted from the hallucination detector 
architecture, and a 5-fold cross-validation ensemble approach.

The model takes demographic and question response features from user profiles, 
creates enhanced embeddings with element-wise products and differences, uses a neural 
network with stronger regularization, trains 5 separate models using cross-validation, 
and combines their predictions for improved accuracy.
"""
...
def train_enhanced_k_fold_models(X, y, device, results_dir="enhanced_models_results", n_splits=5, 
                        batch_size=32, num_epochs=50, patience=10):
    """Train enhanced models using k-fold cross-validation."""
    # Create KFold object
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X)):
        fold_num = fold + 1
        print(f"\n{'='*40}\nTraining Enhanced Fold {fold_num}/{n_splits}\n{'='*40}")
        ...
        model_save_path = os.path.join(results_dir, f"{fold_num}_enhanced_k_fold_model.pth")
...
def plot_k_fold_training_history(fold_metrics, results_dir):
    ...
    plt.title('Enhanced K-Fold Cross-Validation Training History')
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MAE)')
    plt.legend()
    plt.grid(True)
    
    save_path = os.path.join(results_dir, "enhanced_k_fold_training_history.png")
    plt.savefig(save_path)
...
def plot_ensemble_loss_vs_epoch(fold_metrics, results_dir):
    ...
    plt.title('Enhanced Model: Average Training and Validation Loss vs Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MAE)')
    plt.legend()
    plt.grid(True)
    ...
    # Save figure
    save_path = os.path.join(results_dir, "enhanced_ensemble_loss_vs_epoch.png")
    plt.tight_layout()
    plt.savefig(save_path)
...
def plot_fold_performances(fold_metrics, results_dir):
    ...
    ax2.set_ylabel('R² (%)')
    ax1.set_title(f'Enhanced Model: Performance by Fold (Avg MAE: {mean_mae:.2f} ± {std_mae:.2f}, Avg R²: {mean_r2:.4f})')
    ax1.set_xticks(x)
    ...
    # Save figure
    save_path = os.path.join(results_dir, "enhanced_fold_performance_comparison.png")
    plt.tight_layout()
    plt.savefig(save_path)
...
def evaluate_individual_and_ensemble_models(fold_models, X_test, y_test, results_dir):
    ...
    ensemble_metrics_path = os.path.join(results_dir, "enhanced_k_fold_ensemble_accuracy.txt")
    ensemble_plot_path = os.path.join(results_dir, "enhanced_k_fold_ensemble_TrueAgeVSPredictedAge.png")
    error_dist_path = os.path.join(results_dir, "enhanced_k_fold_ensemble_error_distribution.png")
    ...
    ensemble_metrics = calculate_accuracy_metrics(
        ...
        title="Enhanced K-Fold Ensemble"
    )
    ...
    plot_error_distribution(y_test, ensemble_predictions, error_dist_path)
    ...
    for i, model in enumerate(fold_models):
        ...
        print(f"Enhanced Fold {fold_num} Model Test Performance:")
...
def plot_individual_vs_ensemble(individual_metrics, ensemble_error, results_dir):
    ...
    plt.title(f'Enhanced Model: Individual vs Ensemble Performance\nEnsemble improves by {improvement:.2f}%')
    ...
    # Save figure
    save_path = os.path.join(results_dir, "enhanced_individual_vs_ensemble_comparison.png")
    plt.tight_layout()
    plt.savefig(save_path)
...
def main(args):
    """Main execution flow for enhanced k-fold ensemble approach."""
    # Setup environment
    ...
    # Create enhanced features (using hallucination detector architecture)
    ...
    # Train k-fold models with enhanced features and architecture
    print(f"\n{'='*60}\nTraining Enhanced 5-Fold Cross-Validation Models\n{'='*60}")
    fold_models, fold_metrics, overall_val_mae = train_enhanced_k_fold_models(
    ...
    print(f"Enhanced K-Fold ensemble improves over the baseline by {improvement:.2f}%")
    
    # Save summary
    summary_path = os.path.join(results_dir, "enhanced_k_fold_ensemble_summary.txt")
    with open(summary_path, 'w') as f:
        f.write(f"Enhanced K-Fold Ensemble Age Prediction Model Summary\n")
        f.write(f"===============================================\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='VoyageAI Age Prediction Enhanced K-Fold Ensemble Model')
